...
class StaffRegister(BaseClass):
    ...
    def save(self, *args, **kwargs):
        if not self.staff_id: 
            last_id = CounterTable.objects.get(id=0)
            last_id.last_staff_id = last_id.last_staff_id + 1
            last_id.save()
            self.staff_id = 'SM00' + str(last_id.last_staff_id)
            
        if not self.password:
            self.password = generate_unique_id.genrate_password()

        if not self.login_credential_sent:
            subject = f'Login credential from | HELTHIFY for {self.first_name} {self.last_name}'
